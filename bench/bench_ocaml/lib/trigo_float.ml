...
let zero : Float.t = {val_=0 ; pow=0}
let one : Float.t = {val_=1; pow=0}
let minus_one : Float.t = {val_= -1; pow=0}
let two : Float.t = {val_=2; pow=0}
...
(* let pi : Float.t = {val_=31415926535897932384626433832795028841971693993751058209749445923078164062862089986280348253421170679 ; pow= -100} *)
let pi : Float.t = {val_=3 ; pow= -100}
...
(* let sqrt_2 : Float.t = {val_=1414213562373095048801688724209; pow= -30} *)
let sqrt_2 : Float.t = {val_=1; pow= -30}
(* let sqrt_3 : Float.t = {val_=17320508075688772935; pow= -19} *)
let sqrt_3 : Float.t = {val_=1; pow= -19}
...
let sin(a, n : Float.t * nat) : Float.t = 
    ...
    let coef : chebychev_coef = [ 
            {val_=6021947012555463;pow= -16};
            {val_=513625166679107;pow= -15};
            {val_= -10354634426296383;pow= -17};
            {val_= -13732034234358675;pow= -18};
            {val_=13586698380902013;pow= -19};
            {val_=10726309440570181;pow= -20};
            {val_= -7046296793891682;pow= -21};
            {val_= -3963902510811801;pow= -22};
            {val_=194995972671759;pow= -22};
            {val_=8522923894416223;pow= -25};
            {val_= -3351717514643582;pow= -26};
            {val_= -11987008607938776;pow= -28};
            {val_=3835820550079916;pow= -29};
        ]
    in
    ...
    let rec compute (i, acc, t_prev, t_prev_prev, n, coef : nat * Float.t * Float.t * Float.t * nat * chebychev_coef) : Float.t =
        if (i <= n) then
            ...
            compute(i + 1(* 1n *), new_acc, t_next_u, t_prev, n, rest_coef)
        else
            acc
    in
    compute(2(* 2n *), y0, t1, t0, n, coef_from_2)
